...
def return_chart_metadata(
    ...
    """
    chart_metadata = []
    cor_strength_to_r = {"weak": 0.1, "strong": 0.9}
    ...
    for encoding in encodings:
        ...
        if encoding in ["length"]:
            ...
            orientations = [None]
        for correlation_strength in task_details["correlation_strength"]:
            for mark_type in mark_types:
                for graph_num in range(num_graphs):
                    df = generate_correlated_data(
                        num_points=10,
                        r=cor_strength_to_r[correlation_strength],
                        rng=rng,
                    )
                    for orientation in orientations:
                        ...
                        question = return_question(
                            encoding=encoding, mark_type=mark_type
                        )
...
def return_question(encoding: str, mark_type: str):
    ...
    if encoding == "position":
        question = "Are var1 and var2 correlated?"
    elif encoding == "area":
        question = f"Are the areas of the {mark_type}s for var1 and var2 correlated?"
    elif encoding == "length":
        question = "Are the lengths of the bars for var1 and var2 correlated?"
    elif encoding == "color_quantitative":
        question = "Are the lightness of the circles for var1 and var2 correlated?"

    return question


def return_answer_and_options(correlation_strength: str, rng: np.random.Generator):
    if correlation_strength == "weak":
        ...
        raise ValueError("Invalid correlation strength")
